...
class NewtonBundle:
    ...
    usd_path: str
    body_type: str  # "rigid", "cloth", "rod", or anisotropic "cable"
    solver_name: str
    fps: int
    model: Any
    solver: Any
...
def _detect_scene_kind(stage: Usd.Stage) -> str:
    """Return the narrow Palatial scene kind, preserving cable assemblies."""
    from .cable_assembly import is_power_cable_assembly_stage  # noqa: PLC0415

    if is_power_cable_assembly_stage(stage):
        return "cable_assembly"

    rod_found = False
    anisotropic_cable_found = False
    for prim in stage.Traverse():
        ...
        if "NewtonShellAPI" in applied or "NewtonClothAPI" in applied:
            return "cloth"
        intent_attr = prim.GetAttribute("newton:deformable:simulationIntent")
        if intent_attr and intent_attr.HasAuthoredValue():
            intent = str(intent_attr.Get())
            if intent in ("cloth", "shell"):
                return "cloth"
            if intent == "rod":
                rod_found = True
        body_attr = prim.GetAttribute("newton:bodyType")
        if body_attr and body_attr.HasAuthoredValue() and str(body_attr.Get()) == "cloth":
            return "cloth"
        if "NewtonRodAPI" in applied:
            rod_found = True
        if any(
            prim.GetAttribute(name).HasAuthoredValue()
            for name in (
            ...
            anisotropic_cable_found = True

    if anisotropic_cable_found:
        return "cable"
    return "rod" if rod_found else "rigid"


def _detect_body_type(stage: Usd.Stage) -> str:
    """Return the broad runtime family for the detected scene kind."""
...
def _scene_pins_fps(stage: Usd.Stage) -> bool:
    """Return True when the stage explicitly authors its simulation rate."""
    for prim in stage.Traverse():
        if prim.GetTypeName() != "PhysicsScene":
            continue
...
def load(
    ...
    *,
    ...
    fps_pinned = _scene_pins_fps(stage)
    if body_type == "cloth" and not scene_pinned and not solver_override:
        ...
        if style3d_used and getattr(newton.solvers, "SolverStyle3D", None):
            ...
            solver_name = "xpbd"
    elif body_type in ("rod", "cable") and not scene_pinned and not solver_override:
    ...
    if scene_kind == "cable_assembly" and not fps_pinned:
        from .cable_assembly import (  # noqa: PLC0415
