...
class SolverVBD(SolverBase, CouplingInterface):
    ...
    Joint limitations:
        - Supported joint types: BALL, FIXED, FREE, REVOLUTE, PRISMATIC, D6, ROD, ANISOTROPIC_CABLE (legacy AVBD only).
    ...
    def _init_rigid_system(
        ...
        if self.rigid_compliant_alm and np.any(model.joint_type.numpy() == JointType.ANISOTROPIC_CABLE):
            raise ValueError("Anisotropic cable joints require rigid_compliant_alm=False")
        ...
        with wp.ScopedDevice("cpu"):
            ...
            for j in range(n_j):
                if jt[j] == JointType.ROD:
                    ...
                    if tuple(jdof_dim[j]) != (1, 3):
                        raise RuntimeError("ANISOTROPIC_CABLE requires 4 DOF entries in a 1+3 layout")
                    dim_np[j] = 4
        ...
        with wp.ScopedDevice("cpu"):
            ...
            for j in range(n_j):
                if jt[j] == JointType.ROD:
                    ...
                    slot_seeds = (
                        jtarget_ke[dof0] if legacy_lin_k_start is None else min(legacy_lin_k_start, jtarget_ke[dof0]),
                        jtarget_ke[dof0 + 1]
                        if legacy_ang_k_start is None
                        else min(legacy_ang_k_start, jtarget_ke[dof0 + 1]),
                        jtarget_ke[dof0 + 2]
                        if legacy_ang_k_start is None
                        else min(legacy_ang_k_start, jtarget_ke[dof0 + 2]),
                        jtarget_ke[dof0 + 3]
                        if legacy_ang_k_start is None
                        else min(legacy_ang_k_start, jtarget_ke[dof0 + 3]),
                    )
                    for slot_offset, slot_dof in enumerate(slot_dof_indices):
                        slot = c0 + slot_offset
                        joint_material_k_np[slot] = jtarget_ke[slot_dof]
