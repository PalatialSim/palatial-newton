...
def evaluate_joint_force_hessian(
    ...
    if jt == JointType.ANISOTROPIC_CABLE:
        parent_rest = wp.transform_identity()
        if parent_index >= 0:
            parent_rest = body_q_rest[parent_index]
        q_wp_rest = wp.transform_get_rotation(parent_rest * X_pj)
        q_wc_rest = wp.transform_get_rotation(body_q_rest[child_index] * X_cj)
...
def update_duals_joint(
    ...
    if jt == JointType.ANISOTROPIC_CABLE:
        ...
        joint_penalty_k[stretch_idx] = wp.min(
            joint_material_k[stretch_idx], joint_penalty_k[stretch_idx] + beta_lin * wp.length(C_vec_stretch)
        )
        ...
        joint_penalty_k[ang_idx + 0] = wp.min(
            joint_material_k[ang_idx + 0], joint_penalty_k[ang_idx + 0] + beta_ang * wp.abs(kappa[0])
        )
        joint_penalty_k[ang_idx + 1] = wp.min(
            joint_material_k[ang_idx + 1], joint_penalty_k[ang_idx + 1] + beta_ang * wp.abs(kappa[1])
        )
        joint_penalty_k[ang_idx + 2] = wp.min(
            joint_material_k[ang_idx + 2], joint_penalty_k[ang_idx + 2] + beta_ang * wp.abs(kappa[2])
        )
        return
