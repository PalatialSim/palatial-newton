# Example: load a Newton-ready *rod* USDA produced by an external converter and
# simulate the full isotropic-rod pipeline: endpoint twist boundary, connector
# cluster propagation, rod stretch/bend gain overrides, and the textured-tube
# renderer.
#
# Cable USDAs that only need a clamp-and-drop should use
# `example_palatial_load.py` (rod assets there get the lighter cable-twist
# clamp + free-fall flow). This file is the heavy variant for assets that
# exercise the isotropic rod bending model with prescribed endpoint twist.
#
...
#   - body type (newton:bodyType = "rod")
#   - rod material (newton:rod:stretchStiffness / bendStiffness / ...)
#
# Usage:
#   python -m newton.examples.palatial.example_palatial_isotropic <converted_rod.usda>
#       [--steps 600] [--gui] [--substeps N]
#       [--rod-twist-rate RAD_PER_SEC] [--rod-twist-end-time SECONDS]
...
class Example:
    def __init__(
        ...
        bundle = load(
            ...
            device=device,
            table=table,
            rod_textured_tube=rod_textured_tube,
            rod_tube_radial_segments=rod_tube_radial_segments,
        )
        if bundle.body_type != "rod":
            raise RuntimeError(
                "example_palatial_isotropic only handles rod USDAs (got "
                f"body_type={bundle.body_type!r}). Use "
                "example_palatial_load.py for cloth/rigid assets."
            )
        self.bundle = bundle
        ...
        if abs(float(rod_twist_rate)) > 0.0:
            if bundle.body_type != "rod":
                ...
                cluster_body_indices = [
                    body_idx for cluster in cluster_specs for body_idx in cluster.member_body_indices
                ]
                ...
                cluster_local_positions = [
                    local_pos for cluster in cluster_specs for local_pos in cluster.member_local_positions
                ]
                cluster_local_rotations = [
                    local_rot for cluster in cluster_specs for local_rot in cluster.member_local_rotations
                ]
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(prog="example_palatial_isotropic")
    p.add_argument("usd", help="Path to a converted *.newton.usda (rod body type)")
    ...
    p.add_argument(
        "--rod-stretch-damping", type=float, default=None, help="For rod bundles only: override axial stretch damping."
    )
    p.add_argument(
        "--rod-bend-stiffness",
        type=float,
    ...
    p.add_argument(
        "--rod-textured-tube",
        action="store_true",
        help="For rod bundles only: render the rod as a swept textured "
        "tube using the cable mesh's diffuse texture, hiding the "
        "underlying capsule shapes.",
    )
    p.add_argument(
        "--rod-tube-radial-segments",
        type=int,
        default=12,
        help="Radial segment count for --rod-textured-tube (default: 12).",
    )
    ...
    ex = Example(
        ...
        table=table_cfg,
        rod_textured_tube=args.rod_textured_tube,
        rod_tube_radial_segments=args.rod_tube_radial_segments,
    )
    ...
    _auto_top = ex.bundle.body_type == "rod" and (args.record_mp4 or args.gui) and not args.no_auto_camera
