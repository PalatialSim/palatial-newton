...
DEFAULT_GOWN_USD = (
    "/home/achuthan_palatial/Downloads/red_gown/"
    "Newton_assets_6a1a556b4cc488483ba53e0b/"
    "Newton_asset_6a1a556b4cc488483ba53e0b.usda"
)
DEFAULT_GOWN_PRIM = "/World/part_00/GLTF"
...
class Example:
    def __init__(self, viewer, args):
        ...
        self.robot_contact_ke = 1e4
        self.robot_contact_kd = 1e-4
        ...
        # add the red gown.
        # The asset's mesh is authored in meters with metersPerUnit=1 and
        # upAxis=Z. The gown's local +Z is its head-to-toe axis. To drop
        # it onto the table we rotate it -pi/2 about the world X axis so
        # the long axis lies along -world-Y, then translate above the
        # table. The 'scale' parameter on add_cloth_mesh is the
        # meter->centimeter conversion plus a small downscaling so the
        # gown fits inside the table footprint (80cm x 80cm).
        gown_usd_path = getattr(args, "gown_usd", None) or DEFAULT_GOWN_USD
        gown_usd_prim = getattr(args, "gown_prim", None) or DEFAULT_GOWN_PRIM
        usd_stage = Usd.Stage.Open(gown_usd_path)
        if not usd_stage:
        ...
        mesh_indices = gown_mesh.indices
        vertices = [wp.vec3(v) for v in mesh_points]

        # meters -> cm = 100; use 40 to keep the gown roughly the size of
        # the table (~76cm long, ~17cm wide, ~13cm thick after rotation).
        gown_scale_cm = 40.0
        gown_rot = wp.quat_from_axis_angle(wp.vec3(1.0, 0.0, 0.0), -np.pi / 2)
       
        gown_pos = wp.vec3(0.0, -50.0, 40.0)
        ...
        self.viewer.set_camera(wp.vec3(-0.85, 0.85, 1.55), -42.0, -48.0)
    ...
    def create_articulation(self, builder):
        ...
        self.robot_key_poses = np.array(
            [
                ...
                [2, 0.0, -30.0, 35.0, 0.9239, -0.3827, 0.0, 0.0, clamp_close_activation_val],
                [1, 0.0, -30.0, 35.0, 0.9239, -0.3827, 0.0, 0.0, clamp_close_activation_val],
                [1.5, 0.0, -30.0, 35.0, 0.9239, -0.3827, 0.0, 0.0, clamp_close_activation_val],
                [1.5, 0.0, -40.0, 35.0, 0.9239, -0.3827, 0.0, 0.0, clamp_close_activation_val],
                [1.5, 0.0, -40.0, 35.0, 0.9239, -0.3827, 0.0, 0.0, clamp_open_activation_val],
    ...
    def test_final(self):
        # Wider bounds than the T-shirt example: the gown is larger, and
        # without a tuned pick sequence it can flop further off the table.
        p_lower = wp.vec3(-80.0, -120.0, -5.0)
        p_upper = wp.vec3(80.0, 30.0, 120.0)
...
def _run_with_mp4(example, viewer, args):
    """Step the example for args.num_frames, piping each rendered frame to
    ffmpeg so it ends up as an H.264 mp4 at args.record_mp4. Requires a
    ViewerGL (use --viewer gl, optionally --headless) so get_frame() returns
    a populated framebuffer.
    """
    ...
    try:
        # We already emitted frame 0 above, so do num_frames-1 more steps.
        for _ in range(max(0, int(args.num_frames) - 1)):
            example.step()
            example.render()
            proc.stdin.write(viewer.get_frame().numpy().tobytes())
    finally:
        try:
            proc.stdin.close()
...
if __name__ == "__main__":
    ...
    if args.record_mp4:
        _run_with_mp4(example, viewer, args)
    else:
